...
def ingest(db_path: Path) -> None:
    ...
    # Clear existing hunter actions for clean re-import
    old_ids = [r[0] for r in conn.execute(
        "SELECT action_id FROM source_actions WHERE source = 'hunter'"
    ).fetchall()]
    if old_ids:
        ...
        conn.execute("DELETE FROM source_actions WHERE source = 'hunter'")
