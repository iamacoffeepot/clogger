...
RULES: list[FrameRule] = [
    # --- answered ---
    rule("answered",
         rf"^(?:has\s+|have\s+)?(?:not\s+)?answered\s+(?:this\s+)?(?:\{{item\}}\s+)?{ANSWER_OUTCOME}(?:\s+.*)?$",
    ...
    # answered_item
    rule("answered_item",
         rf"^(?:has|have)\s+(?:not\s+)?answered (?:this\s+)?{ITEM_NOUN}\s+"
         rf"(?P<which>correctly|incorrectly|wrong|right)(?:\s+.*)?$",
         lambda m: make_atom(
             "answered_item",
             correct=m.group("which") in ("correctly", "right"),
             neg="not " in m.string[:m.end()],
         )),
    ...
    pre_rule("puzzle_answer",
             rf"^(?:if\s+)?the\s+answer\s+is\s+(?:word\s+)?\w+(?:\s+.*)?$",
             lambda m: make_atom("puzzle_answer")),
    pre_rule("puzzle_answer",
             rf"^(?:if\s+)?(?:the\s+)?correct\s+(?:name|answer|code|word)\s+is\s+"
             rf"(?:chosen|selected|given|entered)(?:\s+.*)?$",
             lambda m: make_atom("puzzle_answer")),
    pre_rule("puzzle_answer",
             rf"^(?:if\s+)?after\s+(?:one|two|three|four|five|\d+)\s+out\s+of\s+"
             rf"(?:one|two|three|four|five|\d+)\s+correct\s+(?:moves?|answers?)(?:\s+.*)?$",
             lambda m: make_atom("puzzle_answer")),
    ...
    pre_rule("puzzle_solved",
             rf"^if (?:the\s+)?puzzle is (?:not\s+)?solved$",
             lambda m: make_atom("puzzle_solved", neg="not " in m.string)),
    pre_rule("puzzle_solved",
             rf"^if the \{{item\}} is (?:not\s+)?solved$",
             lambda m: make_atom("puzzle_solved", neg="not " in m.string)),
    pre_rule("puzzle_solved",
             rf"^if the \{{wiki\}} is (?:not\s+)?solved$",
             lambda m: make_atom("puzzle_solved", neg="not " in m.string)),

    # --- dialogue_state ---
    pre_rule("dialogue_state",
             rf"^(?:if\s+)?(?:not\s+)?all\s+(?:options|questions|topics|dialogue options)\s+"
             rf"have\s+been\s+(?:asked|chosen|selected|exhausted|discussed)(?:\s+.*)?$",
             lambda m: make_atom("dialogue_state", neg="not " in m.string[:10])),
    pre_rule("dialogue_state",
             rf"^(?:if\s+)?(?:the\s+)?player\s+has\s+(?:not\s+)?(?:asked|answered|chosen|exhausted)\s+"
             rf"all\s+(?:options|questions|topics)(?:\s+.*)?$",
    ...
    pre_rule("npc_role",
             rf"^if (?:\w+|\{{npc\}}|\{{monster\}}) is (?:not\s+)?(?:the\s+)?(?P<role>{_NPC_ROLES})(?:\s+.*)?$",
    ...
    # --- non_predicate ---
    rule("non_predicate",
         r"^(?:dialogue\s+\d+|otherwise|else|first time(?:\s+dialogue)?|"
         r"continued|cont\.|laugh|blow kiss|alternative.*)$",
         lambda m: make_atom("non_predicate", kind="marker")),
    rule("non_predicate", r"^\{npc\}$",
         lambda m: make_atom("non_predicate", kind="speaker")),
    pre_rule("non_predicate",
             r"^(?:dialogue\s+\d+|otherwise|else|first time(?:\s+dialogue)?|"
             r"continued|cont\.|laugh|blow kiss|bar item request)$",
             lambda m: make_atom("non_predicate", kind="marker")),
    pre_rule("non_predicate",
             r"^(?:if\s+)?(?:first\s+time\s+speaking|subsequent\s+times?|"
             r"not\s+selected|if\s+selected)$",
             lambda m: make_atom("non_predicate", kind="marker")),
    pre_rule("non_predicate",
             r"^(?:beckon|bow|clap|cry|dance|wave|think|shrug|cheer|laugh|"
             r"yes|no|angry|salute|jig|spin|headbang|yawn|panic|raspberry|blow kiss)$",
             lambda m: make_atom("non_predicate", kind="emote")),
    pre_rule("non_predicate",
             r"^sometimes (?:the following )?dialogue is (?:also )?added$",
             lambda m: make_atom("non_predicate", kind="marker")),
    pre_rule("non_predicate", r"^after a full proper greeting$", lambda m: make_atom("non_predicate", kind="marker")),

    # meta_predicate
    rule("meta_predicate",
         rf"^(?:does not |do not |has not |have not )?(?:satisfy|meet)\s+.*$",
         lambda m: make_atom("meta_predicate")),
    rule("meta_predicate",
         rf"^meets?\s+the\s+(?:quest\s+)?requirements(?:\s+.*)?$",
         lambda m: make_atom("meta_predicate")),
