...
RULES: list[FrameRule] = [
    rule("inventory_space",
         rf"^{NEG}(?:have|has)\s+{INV_CMP}{INV_COUNT}?\s*(?:free\s+)?{INV_NOUN}(?:\s+.*)?$",
         _inv_space),
    ...
    rule("inventory_space",
         rf"^{NEG}(?:have|has)\s+{COUNT}\s+(?:empty\s+|free\s+|open\s+)?{INV_NOUN}(?:\s+.*)?$",
         _inv_space),
    rule("inventory_space",
         rf"^(?P<count>\d+)\s+or\s+more\s+{INV_NOUN}\s+(?:open|free|available)(?:\s+.*)?$",
         _inv_space),
    rule("inventory_space",
         rf"^free\s+inventory\s+(?:space|slots?)$",
         lambda m: make_atom("inventory_space", count=1, neg=False)),
    rule("inventory_space",
         rf"^(?:has\s+|have\s+)?(?:a\s+)?full inventory$",
         lambda m: make_atom("inventory_space", count=0, neg=False)),
    rule("inventory_space",
         rf"^(?:has\s+|have\s+)?insufficient inventory (?:space|slots?)$",
         lambda m: make_atom("inventory_space", count=1, neg=True)),
    rule("inventory_space",
         rf"^(?:has\s+|have\s+)sufficient inventory (?:space|slots?)$",
         lambda m: make_atom("inventory_space", count=1, neg=False)),
    rule("inventory_space",
         rf"^(?:has\s+|have\s+)?open inventory (?:space|slots?)$",
         lambda m: make_atom("inventory_space", count=1, neg=False)),

    pre_rule("inventory_space",
             rf"^without free inventory (?:space|slots?)$",
             lambda m: make_atom("inventory_space", count=1, neg=True)),
    pre_rule("inventory_space",
             rf"^without enough inventory (?:space|slots?)$",
             lambda m: make_atom("inventory_space", count=1, neg=True)),
    pre_rule("inventory_space",
             rf"^with (?:a\s+)?full inventory$",
             lambda m: make_atom("inventory_space", count=0, neg=False)),
    pre_rule("inventory_space",
             rf"^with (?:open |free )?inventory (?:space|slots?)$",
             lambda m: make_atom("inventory_space", count=1, neg=False)),
    pre_rule("inventory_space", rf"^(?:if\s+)?the\s+inventory\s+is\s+(?:not\s+)?full(?:\s+.*)?$",
             lambda m: make_atom("inventory_space", count=0, neg="not " in m.string)),
    pre_rule("inventory_space",
             rf"^(?:if\s+)?there\s+is\s+(?:no\s+)?(?:free\s+|open\s+)?inventory\s+(?:space|slots?)(?:\s+.*)?$",
             lambda m: make_atom(
                 "inventory_space",
                 count=0 if "no " in m.string else 1,
                 neg="no " not in m.string,
             )),
    pre_rule("inventory_space",
             rf"^(?:if\s+)?there\s+(?:are|is)\s+(?:no\s+|not\s+enough\s+)?"
             rf"(?:free\s+|open\s+)?inventory\s+(?:space|slots?)(?:\s+.*)?$",
             lambda m: make_atom(
                 "inventory_space",
                 count=1,
                 neg="no " in m.string or "not " in m.string,
             )),
]
