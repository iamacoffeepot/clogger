...
RULES: list[FrameRule] = [
    ...
    # npc_at_location
    pre_rule("npc_at_location",
             rf"^(?:if\s+)?(?:\{{npc\}}|\{{monster\}})\s+is\s+(?:not\s+)?"
             rf"(?:at|in|near)\s+(?:the\s+)?\{{location\}}(?:\s+.*)?$",
    ...
    # account_state
    rule("account_state",
         rf"^(?:has|have)\s+(?:set\s+up\s+|disabled\s+)?(?:a\s+)?"
         rf"(?:bank pin|authenticator|jagex account|combat checking)(?:\s+.*)?$",
         lambda m: make_atom("account_state")),
    rule("account_state",
         rf"^(?:is|are)\s+playing\s+on\s+(?:a\s+)?(?:jagex account|desktop|mobile)(?:\s+.*)?$",
         lambda m: make_atom("account_state")),
    pre_rule("account_state",
             rf"^if playing on (?:a\s+)?(?:jagex account|desktop|mobile)$",
             lambda m: make_atom("account_state")),
    pre_rule("account_state",
             rf"^(?:if\s+)?(?:not\s+)?restricted\s+from\s+trading(?:\s+.*)?$",
             lambda m: make_atom("account_state")),
    ...
    # in_combat
    rule("in_combat", rf"^{NEG}(?:is\s+)?engaged$",
         lambda m: make_atom("in_combat", neg=bool(m.groupdict().get("neg")))),
    rule("in_combat", rf"^{NEG}(?:is\s+)?in combat$",
         lambda m: make_atom("in_combat", neg=bool(m.groupdict().get("neg")))),

    # world_state (opaque clusters)
    pre_rule("world_state",
             rf"^(?:if\s+)?(?:the\s+)?(?:player's\s+)?chosen boat .*$",
             lambda m: make_atom("world_state", kind="sailing_boat")),
    pre_rule("world_state",
             rf"^(?:if\s+)?sailing level is too low$",
             lambda m: make_atom("world_state", kind="sailing_level")),
    pre_rule("world_state",
             rf"^(?:if\s+)?(?:the\s+)?hat is attuned .*$",
             lambda m: make_atom("world_state", kind="skotizo_hat")),
    pre_rule("world_state",
             rf"^(?:if\s+)?(?:the\s+)?(?:player's\s+)?house is (?:decorated|currently located) .*$",
             lambda m: make_atom("world_state", kind="house_state")),
    pre_rule("world_state",
             rf"^(?:if\s+)?upon spawning .*$",
             lambda m: make_atom("world_state", kind="spawn_state")),
    pre_rule("world_state",
             rf"^(?:if\s+)?(?:while\s+)?waiting for a response .*$",
             lambda m: make_atom("world_state", kind="dialogue_wait")),
]
