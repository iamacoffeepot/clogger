...
RULES: list[FrameRule] = [
    ...
    pre_rule("patch_state", rf"^(?:if\s+)?a\s+patch\s+has\s+not\s+finished\s+growing(?:\s+.*)?$",
             lambda m: make_atom("patch_state", state="growing")),
    pre_rule("patch_state",
             rf"^(?:if\s+)?\{{npc\}}\s+is\s+(?:not\s+)?looking\s+after\s+(?:that|the|this)\s+patch(?:\s+.*)?$",
    ...
    pre_rule("patch_planted",
             rf"^(?:if\s+)?(?:a |an |no |any )?(?:\{{item\}}\s+)?(?:sapling|bush|tree|seedling)?\s*"
             rf"(?:is\s+)?planted\s+in\s+the\s+(?:patch|redwood\s+patch)(?:\s+.*)?$",
             lambda m: make_atom("patch_planted", neg="no " in m.string[:20])),
    pre_rule("patch_planted",
             rf"^(?:if\s+)?no\s+(?:sapling|tree|bush|crop)\s+is\s+planted\s+in\s+the\s+patch(?:\s+.*)?$",
             lambda m: make_atom("patch_planted", neg=True)),
    pre_rule("patch_planted",
             rf"^(?:if\s+)?(?:a |an )\{{item\}}\s+is\s+(?:planted|growing)\s+in\s+the\s+patch(?:\s+.*)?$",
             lambda m: make_atom("patch_planted", neg=False)),
    rule("patch_planted",
         rf"^(?:has|have)\s+(?:not\s+)?(?:something|anything|no\s+crops?)\s+planted(?:\s+.*)?$",
         lambda m: make_atom(
             "patch_planted",
             neg="not " in m.string[:m.end()] or "no " in m.string[:m.end()],
         )),
    rule("patch_planted",
         rf"^(?:has|have)\s+(?:not\s+)?planted\s+(?:a |an |the )?(?:\{{item\}}|seedling|sapling)(?:\s+.*)?$",
         lambda m: make_atom("patch_planted", neg="not " in m.string[:m.end()])),
    ...
    pre_rule("patch_grown",
             rf"^(?:if\s+)?the\s+(?:crop|planted\s+crop|{ITEM_NOUN})\s+has\s+"
             rf"(?:completed\s+growing|died|finished\s+growing)(?:\s+.*)?$",
             lambda m: make_atom(
                 "patch_grown",
                 state="completed" if "died" not in m.string else "died",
             )),
    pre_rule("patch_grown", rf"^(?:if\s+)?the\s+planted\s+crop\s+is\s+diseased(?:\s+.*)?$",
             lambda m: make_atom("patch_grown", state="diseased")),
    pre_rule("patch_grown",
             rf"^(?:if\s+)?(?:a |the )?sapling\s+planted\s+in\s+the\s+patch\s+has\s+"
             rf"(?P<state>matured|died|become\s+diseased)(?:\s+.*)?$",
