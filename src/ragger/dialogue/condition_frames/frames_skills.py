...
RULES: list[FrameRule] = [
    ...
    rule("skill_ge",
         rf"^\{{skill\}}\s+level\s+is\s+at\s+least\s+(?P<level>\d+)(?:\s+.*)?$",
         _skill_ge),
    rule("skill_ge",
         rf"^\{{skill\}}\s+level\s+is\s+too\s+low(?:\s+.*)?$",
         lambda m: make_atom("skill_ge", level=0, neg=True)),

    rule("combat_level",
         rf"^(?:has\s+a\s+)?combat\s+level\s+(?:is\s+)?(?:less|lower)\s+than\s+(?P<level>\d+)(?:\s+.*)?$",
         lambda m: make_atom("combat_level", level=int(m.group("level")), cmp="lt")),
    rule("combat_level",
         rf"^(?:has\s+a\s+)?combat\s+level\s+(?:is\s+)?(?:at\s+least|above|higher\s+than)\s+(?P<level>\d+)(?:\s+.*)?$",
         lambda m: make_atom("combat_level", level=int(m.group("level")), cmp="ge")),
    rule("combat_level",
         rf"^(?:has\s+a\s+)?combat\s+level\s+(?:of\s+)?(?P<level>\d+)\s+or\s+(?:above|higher|more)(?:\s+.*)?$",
         lambda m: make_atom("combat_level", level=int(m.group("level")), cmp="ge")),

    rule("monster_skill_check",
         rf"^\{{monster\}}\s+has\s+a\s+\{{skill\}}\s+level\s+of\s+(?P<level>\d+)(?:\s+.*)?$",
