...
class Recipe:
    ...
    _COLS = "id, members, ticks, notes, facilities"
    ...
    @classmethod
    def search_output(cls, conn: sqlite3.Connection, name: str) -> list[Recipe]:
        """Find recipes whose output item name matches a partial string."""
        rows = conn.execute(
            f"""SELECT r.{cls._COLS.replace(', ', ', r.')}
                FROM recipes r
                JOIN recipe_outputs ro ON ro.recipe_id = r.id
                WHERE ro.item_name LIKE ?
                ORDER BY r.id""",
            (f"%{name}%",),
        ).fetchall()
        return [cls._from_row(row) for row in rows]
