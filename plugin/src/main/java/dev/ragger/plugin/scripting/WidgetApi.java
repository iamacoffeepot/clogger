...
public class WidgetApi {
    ...
    public WidgetApi(Client client) {
        this.client = client;
    }
     ...
     */
    private int get(Lua lua) {
        int groupId = (int) lua.toInteger(2);
        int childId = (int) lua.toInteger(3);
     ...
     */
    private int component(Lua lua) {
        int componentId = (int) lua.toInteger(2);
    ...
    private int children(Lua lua) {
        ...
        lua.createTable(0, 0);
        if (parent == null || parent.isHidden()) return 1;
     ...
     */
    private void pushWidget(Lua lua, Widget w) {
        ...
        Point canvasPos = w.getCanvasLocation();
        if (canvasPos != null) {
    ...
    private int pushChildArray(Lua lua, Widget[] children, int index) {
     ...
     */
    private void registerInterfaceConstants(Lua lua) {
        for (Field field : InterfaceID.class.getDeclaredFields()) {
     ...
     */
    private void registerTypeConstants(Lua lua) {
